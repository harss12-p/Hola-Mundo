...
<section>
 ...
 <form method="post" action="<?= (BASE_URL ? rtrim(BASE_URL,'/') : '') ?>/mensajes/update" enctype="multipart/form-data" class="form">
  <input type="hidden" name="id_mensajes" value="<?= (int)$mensaje['id_mensajes'] ?>"/>
  <label>Título
  ...
  <a class="btn secondary" href="<?= (BASE_URL ? rtrim(BASE_URL,'/') : '') ?>/mensajes/show?id_mensajes=<?= (int)$mensaje['id_mensajes'] ?>">Cancelar</a>
 </form>
</section>